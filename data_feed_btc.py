"""
CryptoTrader AI -- data_feed_btc.py
BTC/GBP live & historical data feed via Kraken (krakenex)
...
log = logging.getLogger("CryptoTrader.DataFeed")
...
def add_indicators(df: pd.DataFrame) -> pd.DataFrame:
    """Calculate all CryptoTrader AI indicators on a full OHLCV DataFrame."""
    if df.empty:
        return df
    ...
    df["tmo_prev"]       = tmo_df["tmo_main"].shift(1)   # prior-bar TMO (rising-momentum check)
    df["chande_mo"]      = _calc_chande(df)
    df["money_flow"]     = _calc_money_flow(df)
    df["atr"]            = _calc_atr(df)   # ATR-14 (GBP) -- market-volatility gate

    return df


def _calc_atr(df: pd.DataFrame, period: int = 14) -> pd.Series:
    """Average True Range in price terms (GBP) over `period` bars.
    Used by the volatility-range filter (System 2 Review, Change 3B, 18 Jul 2026).
    Wilder-style smoothing via an exponential mean with alpha = 1/period."""
...
# ──────────────────────────────────────────────────────────────────────────────
# Main BTCDataFeed class
# ──────────────────────────────────────────────────────────────────────────────

class BTCDataFeed:
    """
    CryptoTrader AI -- BTC/GBP dual-timeframe data feed.
    ...
    Usage:
        ...
        feed.refresh()           # call every 5 mins -- merges new candles
                                 # and recalculates ALL indicators on full data
    """
    ...
    def __init__(self) -> None:
        ...
        log.info(
            "CryptoTrader AI | BTCDataFeed ready | capital=GBP %.2f",
            self.capital_gbp,
        )

    def _fetch_raw(self, tf_label: str) -> pd.DataFrame:
        """Fetch raw OHLC from Kraken without calculating indicators."""
    ...
    def initialise(self) -> None:
        """
        Full historical load for both timeframes.
        Fetches 720 candles and calculates all indicators on the full dataset.
        Call once at startup.
        """
        log.info("=== CryptoTrader AI -- Initialising data feed ===")
        for tf in ("1d", "1h", "5m"):
            ...
            if not df.empty:
                ...
                log.info(
                    "  [%s] %d candles | latest close: GBP %.2f | rsi=%.1f | ssl=%s",
                    tf,
                    len(df),
                    df["close"].iloc[-1],
        ...
        log.info("Data feed ready -- all timeframes loaded with full indicators.")

    def refresh(self) -> None:
        """
        Incremental update -- fetch only new candles since last call,
        merge with existing data, then recalculate ALL indicators on the
        full combined dataset. This ensures indicators never show NaN
        due to insufficient history.
        """
        log.info("=== CryptoTrader AI -- Refreshing ===")
        for tf in ("1d", "1h", "5m"):
            ...
            if not new_df.empty and tf in self._frames:
                ...
                # IMPORTANT: recalculate ALL indicators on the FULL dataset
                # This ensures we never get NaN from insufficient history
            ...
            if tf in self._frames and not self._frames[tf].empty:
                ...
                self._last_since[tf] = int(df.index[-1].timestamp())
                log.info(
                    "  [%s] %d candles total | latest close: GBP %.2f | rsi=%.1f | ssl=%s",
                    tf,
                    len(df),
                    df["close"].iloc[-1],
                    df["rsi"].iloc[-1] if pd.notna(df["rsi"].iloc[-1]) else 0,
    ...
    def get(self, timeframe: str = "5m") -> pd.DataFrame:
        """Return the enriched DataFrame for '1h' or '5m'."""
    ...
    def latest_bar(self, timeframe: str = "5m") -> pd.Series:
        """Return the most recent fully-closed candle with all indicators."""
    ...
    def print_status(self) -> None:
        """Print a readable summary of both timeframes to the terminal."""
        log.info("-" * 60)
        log.info(
            "CryptoTrader AI -- Status  [%s]",
            datetime.now(timezone.utc).strftime("%Y-%m-%d %H:%M UTC"),
        )
        log.info("Paper capital: GBP %.2f", self.capital_gbp)
        for tf in ("1d", "1h", "5m"):
            ...
            log.info(
                "  [%s] close=GBP %.2f  rsi=%.1f  macd=%.4f  ssl=%s  "
...
def fetch_latest_1m_bar(client: krakenex.API) -> Optional[pd.Series]:
    """
    Fetch the most recent closed 1m candle with indicators.
    Used by the position monitor for fast exit detection between 5m candles.
    Single attempt only -- failures return None rather than retrying.
    """
    try:
        ...
        log.warning("1m bar fetch failed: %s", exc)
        return None


if __name__ == "__main__":
    log.info("CryptoTrader AI -- data feed test starting...")
    feed = BTCDataFeed()
    feed.initialise()
    feed.print_status()

    log.info("Testing refresh (simulating next candle)...")
    time.sleep(2)
    feed.refresh()
    feed.print_status()
    ...
    for tf in ("1h", "5m"):
        ...
        print(f"\n=== {tf} -- last 5 candles ===")
        print(
            df[["close", "rsi", "macd", "ssl_bull",
                "tmo_main", "chande_mo", "money_flow"]]
            .tail(5)
            .to_string()
        )

    log.info("Data feed test complete -- no NaN values expected above.")