"""
CryptoTrader AI -- pre_checks_eth.py
...
log = logging.getLogger("CryptoTrader.PreChecks.ETH")
...
def run_all_pre_checks(
    ...
    if current_trade is None:
        checks += [
            # Daily-SSL direction gates REMOVED from the chain (18 Jul 2026,
            # System 2 Review, Change 2/3C): both "ETH Short Only Mode" and the
            # "Daily trend filter" were daily-SSL-driven and too slow for scalping.
            # Direction is now the 1h SSL (primary) + 5m SSL agreement below, with
            # regime + Morgan-SHORT gating handled in main_tidetrader. Daily SSL is
            # context only.
