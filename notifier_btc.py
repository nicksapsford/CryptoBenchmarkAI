"""
CryptoTrader AI -- notifier_btc.py
...
log = logging.getLogger("CryptoTrader.Notifier")
...
def notify_trade_opened(direction: str, entry_price: float, stop_loss: float,
                        ...
                        pair: str = "BTC") -> None:
    _send(
        title=f"CryptoTrader AI - [{pair}] Trade Opened",
        message=(
...
def notify_trade_closed_win(direction: str, close_price: float, pnl_gbp: float,
                             ...
                             pair: str = "BTC") -> None:
    _send(
        title=f"CryptoTrader AI - [{pair}] Trade Won!",
        message=(
...
def notify_trade_closed_loss(direction: str, close_price: float, pnl_gbp: float,
                              ...
                              pair: str = "BTC") -> None:
    _send(
        title=f"CryptoTrader AI - [{pair}] Trade Lost",
        message=(
            f"{direction} closed at GBP {close_price:,.2f} | "
...
def notify_kill_switch_triggered(tier: int, reason: str, wait_hours: int,
                                  ...
                                  pair: str = "BTC") -> None:
    _send(
        title=f"CryptoTrader AI - [{pair}] KILL SWITCH (Tier {tier})",
        message=(
            f"{reason} | "
...
def notify_kill_switch_reset(tier: int, wait_hours: int, capital: float,
                              pair: str = "BTC") -> None:
    _send(
        title=f"CryptoTrader AI - [{pair}] Resuming Trading",
        message=(
...
def notify_tier3_urgent(pair: str = "BTC") -> None:
    _send(
        title=f"CryptoTrader AI - [{pair}] URGENT: Manual Review Needed",
        message=(
...
def notify_system_startup(capital: float, pair: str = "BTC") -> None:
    _send(
        title=f"CryptoTrader AI - [{pair}] System Started",
        message=(
            f"CryptoTrader AI is live. "
            f"Capital: GBP {capital:.2f}. "
...
def notify_economic_block(event_name: str, remain_mins: int) -> None:
    _send(
        title="CryptoTrader AI - Trading Paused",
        message=(
...
def notify_milestone_review(milestone_number: int, pair: str = "BTC") -> None:
    _send(
        title=f"CryptoTrader AI - [{pair}] Milestone Review Complete",
        message=(
...
def notify_daily_summary(date_str: str, trades: int, pnl_gbp: float,
                          ...
                          pair: str = "BTC") -> None:
    _send(
        title=f"CryptoTrader AI - [{pair}] Daily Summary",
        message=(
