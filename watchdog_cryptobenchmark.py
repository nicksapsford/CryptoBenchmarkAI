"""
watchdog_cryptobenchmark.py -- Galahad for CryptoBenchmark (Benchmark Desk).
Launches and supervises main_cryptobenchmark.py: restarts it on unexpected exit,
...
TARGET = BASE_DIR / "main_cryptobenchmark.py"

logging.basicConfig(level=logging.INFO,
                    format="%(asctime)s  %(levelname)-7s [Galahad] %(message)s",
                    datefmt="%Y-%m-%d %H:%M:%S")
logging.Formatter.converter = time.gmtime
log = logging.getLogger("CryptoBenchmark.Watchdog")
