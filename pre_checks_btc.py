"""
CryptoTrader AI -- pre_checks_btc.py
...
log = logging.getLogger("CryptoTrader.PreChecks")
...
# BTC 5m ATR for both engines (BTC-led, unchanged). Mirrors CryptoTrader (5001).
...
# (e.g. backtest / first tick before a price is published).
...
def check_volatility_range(btc_atr, btc_price=None) -> dict:
    ...
    is unavailable (backtest / first tick before a price is published) we fall back to
    the legacy fixed GBP thresholds. Blocks when volatility is too low (flat -- nothing
    to scalp) or too high (flash crash/pump -- too risky). None ATR -> no data, allow."""
...
def run_all_pre_checks(
    ...
    if current_trade is None:
        checks += [
            # Daily trend filter REMOVED from the gate (18 Jul 2026, System 2
            # Review, Change 3C): daily SSL is too slow for scalping. Direction is
            # now driven by the 1h SSL (primary) + 5m SSL agreement below. Daily
            # SSL remains available as context only (check_daily_trend_filter is
            # retained for reference/other callers but no longer gates entries).
            ("SSL agreement",         lambda: check_ssl_agreement(bar_1h, bar_5m)),
...
if __name__ == "__main__":
    ...
    log.info("CryptoTrader AI -- Pre-checks self-test")
